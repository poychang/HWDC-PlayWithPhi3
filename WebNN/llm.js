...
export class LLM {
    ...
    async load(model, options) {
        ...
        const use_local_model = options.use_local_model;
        const hasFP16 = (provider === "wasm") ? false : options.hasFP16;
        this.profiling = options.profiling;

        const model_path = (use_local_model) 
            ? "models/" + model.path 
        ...
        log(`loading... model:${model.name}, provider:${provider}`);
        ...
        this.session = await ort.InferenceSession.create(model_bytes, opt);
